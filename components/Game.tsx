...
const Game:React.FC<Props> = ({setState, state, setData, data, words}) => {
    const [word1, setWord1] = React.useState<number>(5)
    const [word2, setWord2] = React.useState<number>(5)
    ...
    return (
        <div>
            ...
            {
                currentWord === words[1]?
                <h2 className="mt-6 text-center text-4xl font-extrabold text-white">{currentWord}</h2>:
                <h2 className="mt-6 text-center text-4xl font-extrabold text-white">{currentWord}</h2>
            }
        </div>
    )
